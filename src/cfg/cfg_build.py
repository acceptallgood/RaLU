...
class BasicBlock:
    ...
    def stmts_to_code(self) -> str:
        ...
        for stmt in self.stmts:
            ...
            if type(stmt) in [ast.If, ast.For, ast.While, ast.FunctionDef,ast.AsyncFunctionDef]: #需要切割为多个block
                line = line.split('\n')[0].strip()
                if type(stmt) in [ast.FunctionDef,ast.AsyncFunctionDef]:
                    line = line.replace("def", "[DEFINE FUNCTION]").rstrip(":")
                code += line + "\n"
            else: code += line
        return code
...
class CFG:
    ...
    def _traverse_print(self, block: BasicBlock, visited: Set[int] = set()) -> None:
        if block.bid not in visited:
            visited.add(block.bid)
            code = block.stmts_to_code().strip()
            # print("="*20, block.bid, "="*20, len(block.next))
            # print(code)
            
            if len(block.next) > 2:
                print("#"*20, "Error!!! ", block.bid, "#"*20, len(block.next))
                print(code)
                for next_id in block.next:
                    print("-"*10, next_id, "-"*10)
                    print(self.blocks[next_id].stmts_to_code())
                
                raise KeyError("Too many children")
            
            if len(block.next) == 2: #ast.If, ast.For, ast.While
                if len(block.stmts) == 1:
                    code = "`" + code.rstrip(":") + "`"
                else:
                    assert type(block.stmts[-1]) in [ast.If, ast.For, ast.While], f"stmts num: {len(block.stmts)}\n=====\n" + code
                    if len(block.stmts) == 2:
                        self.source.append("RUN `" + astor.to_source(block.stmts[0]).strip() + "`\n$")
                    else:
                        self.source.append("RUN [\n" + "\n".join(code.splitlines()[:-1]).strip() + "\n]$")
                    code = "`" + astor.to_source(block.stmts[-1]).split('\n')[0].strip().rstrip(":") + "`"
                
                true_false = [0, 1]
                edge_0, edge_1 = self.edges[(block.bid, block.next[0])], self.edges[(block.bid, block.next[1])]
                if edge_0 is not None and edge_1 is not None:
                    edge_0, edge_1 = astor.to_source(edge_0).strip(), astor.to_source(edge_1).strip()
                    if edge_0 in code and edge_1 in code:
                        if edge_0 in edge_1: true_false = [1, 0] # edge 0: sublst; edge 1: not sublst; code: `if not sublst`, so edge 1 satisfies the condition
                    elif edge_0 in code:
                        assert edge_1 not in code, f"Edge 0: {edge_0}\nEdge 1: {edge_1}\n{code}"
                    elif edge_1 in code:
                        assert edge_0 not in code, f"Edge 0: {edge_0}\nEdge 1: {edge_1}\n{code}"
                        true_false = [1, 0]
                    #print("*", edge_0, "|||", edge_1, code, true_false[0] == 0, true_false[0] == 1)
                else: self.record = True
                    
                if type(block.stmts[0]) in [ast.For, ast.While]:
                    self.source.append("#LOOP BEGIN# If looping Condition " + code + " is satisfied, then enter the loop:\n$")
                    self._traverse_print(self.blocks[block.next[true_false[0]]], visited=visited)
                    self.source.append("#LOOP END# If not satisfying the looping Condition " + code + ", then jump out of the loop.\n$")
                    self._traverse_print(self.blocks[block.next[true_false[1]]], visited=visited)

                elif type(block.stmts[0] == ast.If):
                    self.source.append("#BRANCH# If Condition " + code + " is satisfied, then ")
                    self._traverse_print(self.blocks[block.next[true_false[0]]], visited=visited)
                    self.source.append("#BRANCH# Otherwise, when Condition " + code + " is not satisfied, then ")
                    self._traverse_print(self.blocks[block.next[true_false[1]]], visited=visited)
                else:
                    raise KeyError(type(block.stmts[0]))
            
            else:
                code = "[\n" + code + "\n]" if len(block.stmts) > 1 else "`" + code + "`"
                self.source.append("RUN " + code + "\n$")
                for next_id in block.next:
                    self._traverse_print(self.blocks[next_id], visited=visited)
    
    def traverse_print(self):
        self._traverse_print(self.start)
        source = self.source
        for k, v in self.func_calls.items():
            func_source = [s for s in v.traverse_print() if len(s.strip()) > 0]
            # print("##", k, func_source)
            # print()
            if len(func_source) > 0:
                func_source[0] = "\n\n#ENTER FUNCTION# " + k +"{\n" + func_source[0]
                func_source[-1] = func_source[-1].rstrip().rstrip("$") + "} #EXIT FUNCTION#$"
                source += func_source
        return source
...
class CFGVisitor(ast.NodeVisitor):
    ...
    def get_function_name(self, node: Type[ast.AST]) -> str:
        if type(node) == ast.Name:
            ...
            if len(str(node.value).strip()) == 0:
                return repr(node.value)
            else: return node.value
        elif type(node) == ast.Call:
            return self.get_function_name(node.func) + "(" + \
            ",".join([str(self.get_function_name(arg))
                                for arg in node.args]) + ")"
        elif type(node) == ast.BinOp:
            op = " "
            if node.op == ast.Sub: op = "-"
            return self.get_function_name(node.left) + op + self.get_function_name(node.right)
    ...
    def visit_Lambda(self, node): # deprecated since there is autopep8
        try:
            ...
            self.add_subgraph(ast.FunctionDef('Anonymous Function', args=node.args, body = [ast.Return(value=node.body)], decorator_list=[], returns=None))
...
def build_from_file(filename):
    try:
        ...
        print(f'Error in source code: {e}')
        exit(1)

    parser = PyParser(source)
    parser.removeCommentsAndDocstrings()
    parser.formatCode()
    return CFGVisitor().build(filename, ast.parse(parser.script))

if __name__ == "__main__":
    cfg = build_from_file("../records/MbppPlus/gpt-4o_241225_1610/Mbpp/126/ini.py")
    entry_point = "sum_of_common_divisors"
    graph = cfg.show(show=False, calls=False)
